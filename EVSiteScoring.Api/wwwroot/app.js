...
function initGoogleMap() {
  ...
  map.addListener('click', event => {
    if (!state.polygonCoords || !isPointInsidePolygon([event.latLng.lng(), event.latLng.lat()], state.polygonCoords)) {
      showError('Place the marker inside the polygon.');
      return;
    }
...
function initLeafletMap() {
  ...
  map.on('click', event => {
    ...
    state.marker = L.marker(event.latlng, { draggable: false }).addTo(map);
    updateScoreButtonState();
  });
}
